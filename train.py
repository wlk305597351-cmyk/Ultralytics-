import warnings, os, sys, argparse, subprocess, csv
    ...
    for k, v in model_state.items():
        if k in ckpt_state:
            if v.shape == ckpt_state[k].shape:
                ...
                skipped += 1
                rows.append((k, str(list(v.shape)),
                             f'shape mismatch: ckpt {list(ckpt_state[k].shape)}'))
        else:
            skipped += 1
...
def _write_registry(name, model_path, cfg, data, epochs, batch, imgsz, device):
    ...
    fieldnames = ['exp_name', 'timestamp', 'git_commit', 'model', 'cfg', 'data',
                  'epochs', 'batch', 'imgsz', 'gpu', 'status',
                  'best_mAP50', 'best_mAP50_95', 'log_path', 'wandb_url', 'notes']
    ...
    with open(registry_path, 'a', newline='') as f:
        ...
        if not file_exists:
            writer.writeheader()
        writer.writerow({
            'exp_name': name, 'timestamp': timestamp, 'git_commit': git_commit,
            'model': model_path, 'cfg': cfg or '', 'data': data,
            'epochs': str(epochs), 'batch': str(batch), 'imgsz': str(imgsz),
            'gpu': device, 'status': 'started',
            'best_mAP50': '', 'best_mAP50_95': '',
            'log_path': f'logs/{name}.log', 'wandb_url': '', 'notes': ''
        })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--optimizer', type=str, default=None,
                        help='优化器（默认: yolo26 用 MuSGD, 其他用 SGD）')
    ...
    parser.add_argument('--dry-run', action='store_true', default=False,
                        help='打印解析后的参数和等效命令，不真正训练')
    parser.add_argument('--afss', action='store_true', default=False,
                        help='开启 AFSS（将来用）')
    ...
    # --- registry 写入 ---
    try:
        _write_registry(args.name, args.model, args.cfg, args.data,
