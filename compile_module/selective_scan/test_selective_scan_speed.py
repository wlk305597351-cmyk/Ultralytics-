...
def build_selective_scan_fn(selective_scan_cuda: object = None, mode="mamba_ssm", tag=None):
    ...
    class SelectiveScanFn(torch.autograd.Function):
        ...
        def backward(ctx, doubt, *args):
            ...
            if MODE in ["mamba_ssm"]:
                ...
                du, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = selective_scan_cuda.bwd(
                    u, delta, A, B, C, D, delta_bias, doubt, x, ctx.delta_softplus, ctx.backnrows
                )
            elif MODE in ["sscorendstate"]:
                du, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = selective_scan_cuda.bwd(
    ...
    def selective_scan_fn(
        ...
        """If return_last_state is True, returns (out, last_state) last_state has shape (batch, dim, dstate). Note that
        the gradient of the last state is not considered in the backward pass.
        """
        return SelectiveScanFn.apply(
            u, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state, nrows, backnrows
        )
...
def selective_scan_easy_v2(
    us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, return_last_state=False, chunksize=3
):
...
def test_speed():
    ...
    D.detach().clone().requires_grad_() if D is not None else None
    z.detach().clone().requires_grad_() if z is not None else None
