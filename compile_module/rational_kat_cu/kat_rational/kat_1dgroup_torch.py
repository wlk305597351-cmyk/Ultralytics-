...
class KAT_Group_Torch(nn.Module):
    ...
    def initialize(self, mode="gelu"):
        ...
        try:
            ...
            self.weight_numerator = nn.Parameter(torch.FloatTensor(weight_numerator), requires_grad=True)
            self.weight_denominator = nn.Parameter(torch.FloatTensor(weight_denominator), requires_grad=True)
