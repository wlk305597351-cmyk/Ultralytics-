...
class My_rational(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        d_x, d_weight_numerator, d_weight_denominator = kat_rational.rational_bwd(
...
class My_rational_optimized(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        x, w_numerator, w_denominator = ctx.saved_tensors
        d_x, d_weight_numerator, d_weight_denominator = kat_rational.rational_bwd_optimized(
            grad_output, x, w_numerator, w_denominator
        )
        return d_x, d_weight_numerator, d_weight_denominator, None
...
def benchmark_fwd_time(x, numerator_weights, denominator_weights):
    ...
    for _ in range(100):
        start = time.time()
        My_rational.apply(x, numerator_weights, denominator_weights)
        torch.cuda.synchronize()
        used_time += time.time() - start
    ...
    for _ in range(100):
        start = time.time()
        My_rational_optimized.apply(x, numerator_weights, denominator_weights)
        torch.cuda.synchronize()
        used_time += time.time() - start
...
if __name__ == "__main__":
    ...
    denominator_weights = nn.Parameter(
        torch.tensor(
            [-0.06675015696494944, 0.17927646217001553, 0.03746682605496631, 1.6561610853276082e-10],
            dtype=torch.float32,
            device="cuda",
        ),
        requires_grad=True,
