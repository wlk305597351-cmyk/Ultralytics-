...
def test_feature_visualization_saves_all_batch_and_channels(tmp_path):
    """Test that feature visualization saves every sample and channel map when visualize is enabled."""
    from ultralytics.utils.plotting import feature_visualization

    x = torch.arange(2 * 3 * 8 * 8, dtype=torch.float32).reshape(2, 3, 8, 8)
    feature_visualization(x, module_type="Conv", stage=5, save_dir=tmp_path)

    stage_dir = tmp_path / "stage5_Conv"
    assert stage_dir.is_dir()
    for b in range(2):
        sample_dir = stage_dir / f"sample_{b:03d}"
        assert sample_dir.is_dir()
        assert (sample_dir / "features.npy").exists()
        saved = np.load(sample_dir / "features.npy")
        assert saved.shape == (3, 8, 8)
        for c in range(3):
            assert (sample_dir / f"ch_{c:04d}.png").exists()
