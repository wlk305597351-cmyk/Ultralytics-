...
from ultralytics.models.yolo.segment.afss_val import AFSSSegmentationEvaluator, aggregate_image_metrics


def make_trainer_stub(tmp_path) -> AFSSSegmentationTrainer:
    trainer = AFSSSegmentationTrainer.__new__(AFSSSegmentationTrainer)
    ...
    trainer.afss_task_name = "segment"
    trainer.afss_dir = tmp_path / "segment" / "train" / "afss"
    trainer.afss_state = {}
    return trainer


def test_segment_aggregate_image_metrics_tracks_box_and_mask():
    ...
    assert result["mask"]["recall"] == 0.75
    assert result["mask"]["precision"] == 0.6


def test_segment_evaluator_collects_joint_payload_by_image_file(tmp_path):
    evaluator = AFSSSegmentationEvaluator(save_dir=tmp_path)
    ...
    assert evaluator.image_results["sample.jpg"]["metrics"]["mask"]["recall"] == 0.75
    assert evaluator.image_results["sample.jpg"]["task_score"] == 0.75


def test_segment_trainer_uses_joint_thresholds_for_state_and_snapshot(tmp_path):
    ...
    trainer._update_afss_state_from_results(
        {
            "easy.jpg": {
                "metrics": {
                    ...
                    "mask": {"precision": 0.92, "recall": 0.90},
                },
                "task_score": 0.90,
            },
            "hard.jpg": {
                "metrics": {
                    ...
                    "mask": {"precision": 0.30, "recall": 0.20},
                },
                "task_score": 0.20,
            },
        },
