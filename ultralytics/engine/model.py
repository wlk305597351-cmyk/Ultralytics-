...
class Model(torch.nn.Module):
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        根据提供的配置文件创建一个新的模型实例。加载模型配置，如果未指定则推断任务类型，
        并使用任务映射表中适当的类来初始化该模型。

        参数 (Args):
            cfg (str): YAML 格式的模型配置文件的路径。
            task (str, 可选): 模型的特定任务。如果为 None，则将从配置中推断。
            model (torch.nn.Module, 可选): 自定义模型实例。如果提供，将使用它而不是创建
                一个新模型。
            verbose (bool): 如果为 True，则在加载期间显示模型信息。

        异常 (Raises):
            ValueError: 如果配置文件无效或无法推断出任务。
            ImportError: 如果没有安装指定任务所需的依赖项。
