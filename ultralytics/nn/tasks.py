...
def parse_model(d, ch, verbose=True):     
    ...
    attention_modules = frozenset(
        {
            ACA, ACAB, CoordAtt, CASAB, ContrastDrivenFeatureAggregation, DeformableLKA, DHPF, EMA, FSA, KSFA, LSKBlock,    
            MCA, MLCA, MultiSEAM, SimAM
        }
    )

    # transformer集合
    transformer_modules = frozenset({CGTA, CirculantAttention, LCGA})  
    ...
    conv_modules = frozenset(
        {  
            ...
            Partial_Conv, PSConv, RepMBConv, ScConv, SFS_Conv, ReparamLargeKernelConv, SMPConv, WideDiverseBranchBlock, WTConv2d,
            DEGConv
        }
    )    
     
    # stem集合
    stem_modules = frozenset({LoGStem, RepStem, SRFD})
  
    # upsample集合     
    upsample_modules = frozenset(    
        {
            CARAFE, Converse2D_Up, DSUB, DySample, EUCB, EUCB_SC, WaveletUnPool     
        }  
    )
    ...
    module_modules = frozenset(   
        {    
            ...
            ELGCA_EncoderBlock, ESCBlock, EVA, Faster_Block, Faster_Block_CGLU, Faster_CGA_Block, FAT_Block, FCM, FMA, GLGM, HFRB, InceptionDWBlock, IEL, GLSA,  
            ...
            SparseMambaBlock, MSInit, PFG, LFP, AMSI, CMSI, FMSI, HOIE, ADIE, HPFGA, DPFGA, SFEB
        }     
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args 
        ...
        # ------------------- featurefusion -------------------     
        elif m in frozenset({CAFM, CIDAF, ContextGuideFusionModule, DynamicAlignFusion, DPCF, ERM, HAFFormer, HFFE, LCA, MFM, MFPM, MultiScalePCA, MultiScalePCA_Down, MSAM_P5, 
                            GDSAFusion, MultiScaleGatedAttn, WFU, CGAFusion, SDFM, PSFM, WDAF, SFSFusion, FAAFusion}):  
